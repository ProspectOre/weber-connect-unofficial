...
def load_or_create_pairing_keys(
    ...
    if path.exists() and not reset_key:
        keys: dict[str, Any] = json.loads(path.read_text(encoding="utf-8"))
...
async def pair_once(args: argparse.Namespace, keys: dict[str, Any]) -> dict[str, Any]:
    ...
    async with BleakClient(args.address, timeout=args.timeout) as client:
        ...
        async def poll_response(
            duration: float, stop: Callable[[], bool], interval: float = 1.0
        ) -> None:
        ...
        for _ in range(3):
            ...
            def greeting_done(before: int = errors_before) -> bool:
                return (
            ...
            await poll_response(10.0, stop=greeting_done)
